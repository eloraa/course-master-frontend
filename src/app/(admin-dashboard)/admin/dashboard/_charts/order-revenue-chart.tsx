...
import { useOrderStatistics } from '@/data/admin/statistics';
...
export const OrderRevenueChart = ({ dateRange: externalDateRange, onDateRangeChange, standalone = true }: OrderRevenueChartProps = {}) => {
  ...
  const { data: statsData, isLoading } = useOrderStatistics(convertDateRangeToStrings(dateRange));
      ...
      <div className="absolute w-full top-0 pr-8">
        <ChartContainer config={chartConfig} className="h-[80px] w-full cursor-crosshair [&_svg]:cursor-crosshair">
          ...
          >
            ...
            <YAxis tickLine={false} axisLine={false} className="text-xs" tickFormatter={formatCurrency} />
            <ChartTooltip content={<ChartTooltipContent formatter={value => formatCurrency(Number(value))} />} />
            <Area type="step" dataKey="revenue" stroke="var(--color-revenue)" strokeWidth={2} fill="url(#colorOrderRevenue)" fillOpacity={1} className="cursor-crosshair" />
            {refAreaLeft && refAreaRight && <ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} fill="hsl(var(--chart-1))" fillOpacity={0.3} />}
          </AreaChart>
        </ChartContainer>
      </div>
