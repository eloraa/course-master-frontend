...
import { useCourseStatistics } from '@/data/admin/statistics';
...
export const EnrollmentChart = ({ dateRange: externalDateRange, onDateRangeChange }: EnrollmentChartProps = {}) => {
  ...
  const { data: statsData, isLoading } = useCourseStatistics(convertDateRangeToStrings(dateRange));
    ...
    return [
      {
        ...
        enrollments: statsData.data.totalEnrollments,
      },
    ];
  }, [statsData, dateRange]);
          ...
          >
            <defs>
              <linearGradient id="colorEnrollment" x1="0" y1="0" x2="0" y2="1">
                ...
                <stop offset="100%" stopColor="var(--color-enrollments)" stopOpacity={0.1} />
              </linearGradient>
            </defs>
            ...
            <Area type="step" dataKey="enrollments" stroke="var(--color-enrollments)" strokeWidth={2} fill="url(#colorEnrollment)" fillOpacity={1} className="cursor-crosshair" />
            {refAreaLeft && refAreaRight && <ReferenceArea x1={refAreaLeft} x2={refAreaRight} strokeOpacity={0.3} fill="hsl(var(--chart-2))" fillOpacity={0.3} />}
          </AreaChart>
        </ChartContainer>
      </div>
